...
**当前状态：2.0 所有 P0/P1 任务已全部完成，后续增强已覆盖 DaySlot 排班、图片上传、排序、定额款式价格/时长和 GHCR 部署链路。**
...
- ✅ T0-2 技术方案冻结：顾客主身份切换、定额款式模型、旧邮箱下线路径已确认
...
- ✅ T1-2 数据迁移：迁移文件已生成并跟踪，后续已追加定额款式加项、DaySlot、排序和系统设置长文本迁移
- ✅ T1-3 数据访问层：顾客查找逻辑以 LINE 为主，预约创建支持记录定额款式来源
...
- ✅ T3-2 定额款式字段：封面图 URL、标题、说明、价格文案、分类、关联套餐
- ✅ T3-3 定额款式 API：admin CRUD + public 查询（分类筛选、上下架过滤、排序）

## 5. 已完成任务（P1 — 前台首页与预约流程）

- ✅ T4-1 定额款式页支持分类筛选；当前 `/` 默认进入 menu，`/?view=design` 显示定额款式
- ✅ T4-2 定额款式项点击进入固定套餐预约页，不提供套餐切换
...
- ✅ T6-2 预约页：来源定额款式项展示、顾客 LINE 关联情况
...
| 多图定额款式详情 | 当前每个定额款式项仅单图 |

## 10. 2026-05 至 2026-06 追加完成项

- ✅ 定额款式固定/可选加项、加项数量和最大数量
- ✅ 定额款式自定义价格、折扣展示和自定义时长
- ✅ 套餐/加项排序与顾客侧排序一致
- ✅ 本地图片上传与上传图片 API
- ✅ 基于 `DaySlot` 的拖拽排班日历
