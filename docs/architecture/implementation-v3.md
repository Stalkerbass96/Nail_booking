...
1. Prisma schema 与数据库迁移链路已落地，迁移已覆盖 LINE-first、定额套餐加项、自定义价格/时长、DaySlot、排序字段和长文本系统设置。
2. 后台登录鉴权已落地，使用 JWT cookie session + middleware 保护管理页面与管理 API。
3. 前台定额套餐首页（分类筛选、宫格布局），定额套餐项关联套餐，点击进入固定套餐预约流程。
4. 可用时段计算已切换到 `DaySlot`；预约提交会校验开放 slot、封锁区间和预约冲突。
5. 预约状态流转已实现：`pending`、`confirmed`、`completed`、`canceled`。
6. 后台已支持：定额套餐管理、预约管理、顾客管理、LINE 会话管理、拖拽排班、分类/套餐/加项管理、排序、图片上传、积分查看与扣减、系统设置。
7. LINE 完整接入：Webhook follow/unfollow/message/accountLink、自动建档（`lead` 顾客）、首次推送定额套餐链接、预约待确认通知、预约已确认通知、后台 1 对 1 文本会话。
