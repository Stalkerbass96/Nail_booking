...
本手册目标：你在一台新的 Linux 主机上，仅安装 Docker + Docker Compose，也能把项目完整跑起来。
...
- `ADMIN_AUTH_SECRET`：改成随机强密钥（32+ 位）
- `ADMIN_SEED_PASSWORD`：改成你的后台初始密码
- `CRON_SECRET`：改成随机字符串
...
- `postgres`：数据库
- `app`：Next.js 应用（自动执行 `prisma migrate deploy`）
- `auto-cancel-worker`：每 5 分钟执行一次待确认超时取消
...
- Password: `.env.deploy` 里的 `ADMIN_SEED_PASSWORD`
...
- 应用容器启动会自动执行迁移命令。
- `db:seed` 不建议每次发布都执行，避免意外重置管理员密码。
