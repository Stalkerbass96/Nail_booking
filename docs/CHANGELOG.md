...
### 顾客积分与前台排序
- 通过有效 LINE 专属链接识别到的客户，可在图墙、套餐、详情和 option 页面顶栏查看当前积分余额；预约填写页不显示，0 分也会显示。
...
- 加项支持数量、最大数量与 stepper；图墙固定组合通过 `ShowcaseItemAddon` 建模。
- 图墙支持自定义价格、折扣展示、隐藏加项明细、可选加项与长列表滚动。
- 图墙预约表单正确展示包含固定加项后的总价和总时长。
- 前台新增 `/addons` 加项目录页。
