...
### 顾客积分与前台排序
- 通过有效 LINE 专属链接识别到的客户，可在定额套餐、套餐、详情和 option 页面顶栏查看当前积分余额；预约填写页不显示，0 分也会显示。
...
- 定额套餐首页查询只选择必要字段，降低迁移未完全执行时的页面风险。
- 后台导航压缩为更适合运营使用的布局。
- 系统设置新增可编辑 LINE 消息模板。
- LINE 预约通知固定使用日文模板；后台改期时同步按 `DaySlot` 校验。
- 套餐和加项新增 `sortOrder` 字段、迁移和上下移动 UI。
- 定额套餐项新增 `customDurationMin`，支持按款式覆盖标准时长。
...
- 加项支持数量、最大数量与 stepper；定额套餐固定组合通过 `ShowcaseItemAddon` 建模。
- 定额套餐支持自定义价格、折扣展示、隐藏加项明细、可选加项与长列表滚动。
- 定额套餐预约表单正确展示包含固定加项后的总价和总时长。
- 前台新增 `/addons` 加项目录页。
...
- `follow` 事件自动创建顾客档案（`customerType=lead`）并仅首次推送定额套餐首页链接。
- 定额套餐前台首页完成切换，支持分类筛选，点击定额套餐项进入固定套餐预约流程。
