...
- LINE 首次自动发送定额套餐预约链接
- 前台为定额套餐式作品展示页，每个作品关联后台套餐
- 顾客从定额套餐选款进入预约，提交后 LINE 收到待确认通知
- 店长后台确认后，顾客收到 LINE 已确认通知
- 后台可维护定额套餐、排班、套餐、顾客、积分与 LINE 1 对 1 对话
- 中日双语（`?lang=zh|ja`）
...
- 2.0 LINE-first 主链路已落地：LINE 加友、定额套餐选款、预约提交、后台确认、LINE 通知
- UI 已完成暖色系编辑风格重构
- 定额套餐支持自定义价格、自定义时长、固定/可选加项和图片上传
- 排班已切换到 `DaySlot` 拖拽日历，前台预约按开放 slot 校验
- 套餐、加项和前台展示支持 `sortOrder`
- LINE follow 自动建档、欢迎推送、预约通知已实现
- 后台支持定额套餐、套餐、加项、排班、顾客、积分、系统设置和 LINE 1 对 1 会话
...
| 页面 | URL |
|------|-----|
| 前台定额套餐 | `http://localhost:3000` |
| 后台登录 | `http://localhost:3000/admin/login` |
| 定额套餐管理 | `http://localhost:3000/admin/showcase` |
