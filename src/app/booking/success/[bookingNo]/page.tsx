...
export default async function BookingSuccessPage({ params, searchParams }: Props) {
  ...
  return (
    <main className="mx-auto flex min-h-[62vh] w-full max-w-3xl items-center px-4 py-8 sm:px-5 sm:py-10 md:px-8">
      <section className="ui-card w-full shadow-[0_20px_55px_rgba(120,25,55,0.12)]">
        ...
        <div className="mt-7 grid gap-3 sm:flex sm:flex-wrap">
          <Link className="ui-btn-primary px-5 py-2.5" href={`/booking/lookup?lang=${lang}`}>
            {isJa ? "予約を確認" : "查询预约状态"}
          </Link>
          <Link className="ui-btn-secondary px-5 py-2.5" href={`/services?lang=${lang}`}>
            {isJa ? "メニューへ戻る" : "继续浏览套餐"}
          </Link>
        </div>
      </section>
